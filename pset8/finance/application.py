...
def buy():
    ...
    if request.method == 'POST':
        ...
        db.execute("UPDATE users SET cash = :cash WHERE id = :user", user=session['user_id'], cash=cash_left)
...
def sell():
    ...
    if request.method == 'POST':
        ...
        db.execute('UPDATE users SET cash = :cash WHERE id = :user', user=session['user_id'], cash=cash_after)
    ...
    # request method == GET
        ...
        stock_info = db.execute('SELECT symbol, amount FROM stocks WHERE user_id = :user', user=session['user_id'])
        stocks = {}
